import { NextAuthOptions, Profile as NextAuthProfile } from "next-auth";
import AzureADProvider from "next-auth/providers/azure-ad";
import prisma from "@/lib/prisma";

// Define default prompts to be seeded for new users
// This defaultPromptsToSeed constant needs to be defined here or imported if it's complex
// For simplicity, if it's very large, consider moving it to its own file and importing it.
// Assuming it's the same as what was in the [...nextauth]/route.ts file:
...
export const authOptions: NextAuthOptions = {
    ...
    AzureADProvider({
      ...
      profile(profile: NextAuthProfile & { oid?: string; preferred_username?: string }) {
        return {
          ...
          name: profile.name,
          image: null, // Or map the actual image if available from Azure AD profile
        };
      },
    }),
  ],
  callbacks: {
    async jwt({ token, user, account, profile: rawProfile }) {
      ...
      if (account && user && profile && profile.oid) {
        token.userId = user.id; // user.id from NextAuth User (which is profile.oid)
        token.azureOid = profile.oid; // Explicitly store Azure OID if needed elsewhere
        
        try {
          const existingUser = await prisma.user.findUnique({
            where: { id: user.id }, // user.id here is critical
          });

          if (!existingUser) {
            const newUser = await prisma.user.create({
              data: {
                id: user.id, // Use the id from NextAuth User (which is profile.oid)
                email: user.email!,
                name: user.name,
                // image: user.image, // if you map it in profile callback
              },
            });
            ...
            const promptsToCreate = defaultPromptsToSeed.map(p => ({
              ...p,
              userId: newUser.id, // Link prompts to the new user's ID
            }));
            ...
            try {
              ...
              // Log specific error if seeding fails, e.g. unique constraint on prompt name for user
              console.warn("Failed to seed prompts (possibly already exist or other constraint violation):", e);
            }

          } else {
            // Optionally update existing user's details if they changed in Azure AD
            let updateData: { email?: string; name?: string | null /*; image?: string | null*/ } = {};
            if (user.email && existingUser.email !== user.email) {
              updateData.email = user.email;
            }
            if (user.name !== undefined && existingUser.name !== user.name) { // Check for undefined to allow clearing name
              updateData.name = user.name === null ? null : user.name;
            }
            // if (user.image !== undefined && existingUser.image !== user.image) {
            //   updateData.image = user.image === null ? null : user.image;
            // }
    ...
    async session({ session, token }) {
      // Pass 'userId' from JWT token to session.user.id
  ...
  // If you have custom pages, you can specify them here, otherwise defaults are used
  // pages: {
  //   signIn: '/auth/signin', // Example: if you have a custom sign-in page
  // },
  // secret: process.env.NEXTAUTH_SECRET, // Make sure NEXTAUTH_SECRET is set in .env for production
